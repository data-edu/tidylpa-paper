...
write_rds(curry_mac_mclust_j, "cm_mclust_r.rds")
...
write_rds(curry_mac_mclust_j, "cm_mclust_j.rds")
...
curry_mac_mplus_solutions <- curry_mac %>% 
  select(KinshipR:PossessionR, 
         KinshipJ:PossessionJ) %>% 
  estimate_profiles(models = c(1, 2, 3, 6),
...
write_rds(curry_mac_mplus_solutions, "cm_mplus.rds")
...
```{r, m6ps-mplus, error = TRUE}
curry_mac_m6p2_mplus <- curry_mac %>% 
  select(KinshipR:PossessionR, 
         KinshipJ:PossessionJ) %>% 
...
# equivalent to model 6

plot_profiles(curry_mac_m6p2_mplus)
```
